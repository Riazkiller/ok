...
░ ▒▓ ░▒▓░   ░▓      ▒▒   ▓▒█░   ░▒▒ ▓░▒░▒
  ...
  *au : RIAZKHAN
    *fb : RIAZKHAN
   *gh : github.com/Riazkiller   
*yt : youtube.com/it's riaz yt
"""
ips=None
try:
...
class crack:
		...
		while True:
			...
			if f=="":continue
				...
				try:
					...
					menu()
					continue
				print(' *•> Aktifkan Mode Pesawat 2 Detik Jika Tidak Ada RIAZ')
				print('\n *--> RIAZ Ok Tersimpan Di Ok.txt')
				print(' *--> RIAZ Cp Tersimpan Di Cp.txt\n')
	...
	def pwlist(self):
		...
		if len(self.pw) ==0:
			...
			for i in self.fl:
				i.update({"pw":self.pw})
			print('\n *•> Aktifkan Mode Pesawat 2 Detik Jika Tidak Ada RIAZ')
                        print('\n *--> RIAZ Ok Tersimpan Di Ok.txt')
                        print(' *--> RIAZ Cp Tersimpan Di Cp.txt\n')
